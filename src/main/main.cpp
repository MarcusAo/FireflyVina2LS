			   ...
			   int elite)
{
	...
	else
	{
		...
		if (clustering == 1)
		{
			...
			refine_structure(m, prec, nc, out_cont_2[i], authentic_v, par.firefly.ssd_par.evals);
			VINA_FOR_IN(i, out_cont_3)
			refine_structure(m, prec, nc, out_cont_3[i], authentic_v, par.firefly.ssd_par.evals);
		}
		...
		if (clustering == 1)
		{
			...
			VINA_FOR_IN(i, out_cont_2)
			{
				if (how_many_2 >= num_modes || !not_max(out_cont_2[i].e) || out_cont_2[i].e > out_cont_2[0].e + energy_range)
					break; // check energy_range sanity FIXME
				++how_many_2;
				log << std::setw(4) << i + 1
			...
			VINA_FOR_IN(i, out_cont_3)
			{
				if (how_many_3 >= num_modes || !not_max(out_cont_3[i].e) || out_cont_3[i].e > out_cont_3[0].e + energy_range)
					break; // check energy_range sanity FIXME
				++how_many_3;
				log << std::setw(4) << i + 1
